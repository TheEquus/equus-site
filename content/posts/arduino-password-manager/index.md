...
Link to Github repo with code and instructions to set up dropping on 27/10/21
...
The result is then fed into `random.seed()`, which will act as the seed to generate a large integer, which then gets decoded to the password.  
...
- Both the Arduino and Python code could be cleaned up to be more standard/uniform
...
### The Audio
The audio was the part that received the least attention, as it mainly involved dragging and dropping the audio files into the editor, with minor changes to volume, and perhaps the occasional fade in/out. Music was pulled from copyright free music sites (except for Sandstorm of course).
...
## Conclusion
Would I ever use this system myself? Probably not, however it's a ton of fun to make. But name a more fun password manager, that not only offers the lights, beats, and candy like this system, but also has a fun 5 minute video to accompany it?
